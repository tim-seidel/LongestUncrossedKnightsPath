...
/**
  ...
  */
std::pair<uint8_t, uint8_t> Chessboard::doMove(std::pair<uint8_t, uint8_t> source, uint8_t angle) const{
    switch(angle) {
        case 0b00000001:
...
/**
  ...
  */
uint8_t Chessboard::rotate180(uint8_t angle) const{
...
/**
  ...
  */
bool Chessboard::isFinishable(std::pair<uint8_t, uint8_t> source, std::pair<uint8_t, uint8_t> target, uint8_t outgoing_angle) const{
  return (target.first == root.first && target.second == root.second)
		  && !(source.first == root.first && source.second == root.second)
		  && !isConnectionCrossesEdge(source, outgoing_angle);
}
